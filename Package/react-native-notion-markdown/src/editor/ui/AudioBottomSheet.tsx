...
import { AudioFileIcon, CancelIcon, MicrophoneIcon, StopIcon } from "./audioIcons.tsx";
...
type AudioIconName = "choose" | "record" | "stop" | "cancel";
...
function getRecordingIcon(
    action: "record" | "stop" | "cancel",
    components: NotionEditorComponents | undefined
): ComponentType<NotionEditorIconProps>
{
    const override = action === "record"
        ? components?.record ?? components?.speech
        : action === "stop" ? components?.stop : components?.cancel ?? components?.close;
    ...
    return action === "record" ? MicrophoneIcon : action === "stop" ? StopIcon : CancelIcon;
}
...
function WaveformBar({ amplitude, isRecording, index }: { readonly amplitude: number; readonly isRecording: boolean; readonly index: number })
{
    ...
    const animateQuietPulse = useCallback(() =>
    {
    ...
    useEffect(() =>
    {
        ...
        {
            return () => quietPulse.stopAnimation();
        }
        animateQuietPulse();
        return () => quietPulse.stopAnimation();
    }, [ animateQuietPulse, isQuiet, isRecording, quietPulse ]);
    ...
    const quietScale = quietPulse.interpolate({
        inputRange: [ 0, 1 ],
        outputRange: [ 0, isRecording && isQuiet ? 0.12 : 0 ]
    });
    const scale = Animated.add(soundScale, quietScale);
        ...
        <View style={ styles.modalRoot }>
            ...
            <View accessibilityViewIsModal
                ...
                { phase === "confirmation" && recordingAsset !== undefined && <View style={ styles.confirmation }>
                    <Text style={ [ styles.fileName, { color: foreground } ] }>{ recordingAsset.fileName ?? labels.noAudio }</Text>
                    <Text style={ [ styles.duration, { color: muted } ] }>{ formatDuration(recordingAsset.duration ?? 0) }</Text>
                    <Pressable accessibilityLabel={ labels.preview }
                        onPress={ togglePreview }
                        style={ styles.preview }>
                        <Text style={ [ styles.previewText, { color: accent } ] }>{ labels.preview } · { previewStatus.playing ? labels.pause : labels.play }</Text>
                    </Pressable>
                    { error !== undefined && <Text style={ styles.error }>{ error }</Text> }
                    <View style={ styles.confirmationActions }>
                        <Pressable onPress={ cancel }
                            style={ styles.cancelAction }><Text style={ [ styles.cancelText, { color: muted } ] }>{ labels.cancel }</Text></Pressable>
                        <Pressable onPress={ () => void confirm() }
                            style={ [ styles.confirmAction, { backgroundColor: accent } ] }><Text style={ styles.confirmText }>{ replacement ? labels.replace : labels.confirm }</Text></Pressable>
                    </View>
                </View> }
...
const styles = StyleSheet.create({
    cancelAction: { alignItems: "center", flex: 1, justifyContent: "center", minHeight: 48 },
    ...
    confirmationActions: { flexDirection: "row", gap: 12, marginTop: 22, width: "100%" },
    confirmAction: { alignItems: "center", borderRadius: 10, flex: 1, justifyContent: "center", minHeight: 48 },
    ...
    duration: { fontSize: 17, fontVariant: [ "tabular-nums" ], marginTop: 8 },
    error: { color: "#C23B32", fontSize: 13, marginTop: 12, textAlign: "center" },
    fileName: { fontSize: 17, fontWeight: "600", maxWidth: "90%" },
