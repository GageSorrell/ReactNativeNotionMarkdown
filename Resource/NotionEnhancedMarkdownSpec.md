---
copyright: This document was exported from the Notion documentation website, and its content belongs to Notion LLC.  All content below this frontmatter is faithful to the original source.
---
# Enhanced markdown format

> Reference for the Markdown-formatted content format used by the markdown content endpoints.

## Overview

Enhanced markdown (also called "Markdown-formatted content") is an extended Markdown format that supports all Markdown block and rich text types. It is used by the markdown content endpoints: `POST /v1/pages` (via the `markdown` body param), `GET /v1/pages/:page_id/markdown`, and `PATCH /v1/pages/:page_id/markdown`.

This format extends standard Markdown with XML-like tags and attribute lists to represent Markdown-specific features such as callouts, toggles, columns, mentions, and block-level colors.
...
A Markdown page with a heading, a callout, a to-do list, and a code block renders as:
