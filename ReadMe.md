...
## Milestone 1: foundation and native editor transport

Open **Milestone 1 / Native editor transport / Three Blocks** in Storybook. The prototype starts with a text block, a heading, and another text block. Its diagnostic header displays native revisions, composing ranges, and block-relative UTF-16 selection endpoints. The diagnostic actions split/merge blocks, insert soft breaks, select all, and replace the document with a new epoch. Copy/cut/paste work from the toolbar and Android's system selection menu; the clipboard contains interoperable plain text and a versioned package-specific fragment preserving text/heading types. The Insert panel bookmarks selection by retaining the native buffer, hides the keyboard, and restores editing when closed. The Aa button demonstrates switching toolbar modes, not the later formatting catalog.

The editor intentionally uses a single coordinated Android `EditText`/`Editable`, with block boundaries mapped to stable editor IDs and headings rendered using spans. Android owns its continuous selection handles, scrolling, composing buffer, and `InputConnection`. JavaScript accepts revisioned snapshots without resetting the buffer; replacement changes epoch, restarts input, and rejects writes from old input connections. This is an editor transport model, **not** the milestone-2 public Markdown document model or the complete editor. Selection coordination between separately mounted native fields, heterogeneous/atomic blocks, and the full formatting UI remain to be addressed before expanding the catalog.
...
The editor UI uses Lucide icons when `lucide-react-native` is installed. Lucide is an optional peer dependency; applications can omit it and provide `MarkdownEditorComponents` icon overrides or use the accessible text fallback.
...
`parseMarkdown` returns a versioned recursive `MarkdownDocument` and source-located diagnostics for the enhanced Markdown format in `Local/MarkdownSpec.md`.

`serializeMarkdown` produces canonical tab-indented enhanced Markdown, while `fromMarkdownBlocks` and `toMarkdownBlocks` import and export SDK-shaped block trees without network access.

`createMarkdownEditor` provides immutable transactions, grouped undo/redo, revisioned subscriptions, Markdown serialization on demand, and selection mapping across nested blocks, captions, and table cells.

`MarkdownEditorProvider`, `useMarkdownEditor`, and `useMarkdownEditorState` expose the same store to composable React editor surfaces.

SDK conversion diagnostics are returned by default and become `MarkdownConversionError` exceptions when `toMarkdownBlocks` is called with `{ strict: true }`.
...
Run `npm run prebuild:android --workspace markdown-storybook` and then `npm run android --workspace markdown-storybook`. The first build generates the native Android project and installs the development client.

## Subsequent launches

Run `npm run start --workspace markdown-storybook` and open the installed development build, or use `npm run android --workspace markdown-storybook` to build and launch it directly.
